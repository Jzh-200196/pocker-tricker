/**
 * @license
 * SPDX-License-Identifier: Apache-2.0
 */

import { useState, useEffect, useCallback, useMemo } from 'react';
import { drawXCards } from '../utils/cardGenerator';
import { detectCombo } from '../utils/comboDetector';
import { shuffle } from '../lib/utils';
import { ENEMIES } from '../constants/enemies';
import { 
  BattleState, 
  AnomalyMechanism, 
  DamagePopup, 
  BattleNotification 
} from '../types/battle';
import { Card } from '../types/card';
import { RunState } from '../types/game';

interface UseBattleManagerProps {
  run: RunState;
  onVictory: (loot: any, endHp: number) => void;
  onDefeat: () => void;
}

/**
 * useBattleManager - 核心业务逻辑管理库
 * 
 * 100% 逻辑保真版：
 * - 包含所有伤害计算、Combo 检测、遗物加成逻辑。
 * - 严格保留批量 Pin (2项限额)、重抽 (2次限额)、防脸黑补牌。
 */
export function useBattleManager({ run, onVictory, onDefeat }: UseBattleManagerProps) {
  const [battle, setBattle] = useState<BattleState | null>(null);
  const [isEnemyShaking, setIsEnemyShaking] = useState(false);
  const [isPlayerShaking, setIsPlayerShaking] = useState(false);
  const [showExplosion, setShowExplosion] = useState(false);
  const [logs, setLogs] = useState<string[]>([]);
  const [damagePopups, setDamagePopups] = useState<DamagePopup[]>([]);
  const [notifications, setNotifications] = useState<BattleNotification[]>([]);
  const [isHandShaking, setIsHandShaking] = useState(false);
  const [isInvalidDeck, setIsInvalidDeck] = useState(false);
  const [isInvalidCombo, setIsInvalidCombo] = useState(false);
  const [invalidTriggerCount, setInvalidTriggerCount] = useState(0);
  const [showInvalidFeedback, setShowInvalidFeedback] = useState(false);
  const [victoryLoot, setVictoryLoot] = useState<{ gold: number, playerHp: number } | null>(null);
  const [battlePrompt, setBattlePrompt] = useState<string | null>(null);
  
  // 交互同步状态 (React 层)
  const [isDragging, setIsDragging] = useState(false);
  const [dragMode, setDragMode] = useState<'select' | 'deselect' | null>(null);
  const [hoveredCardId, setHoveredCardId] = useState<string | null>(null);

  const generateEnemyIntent = useCallback((turn: number) => {
    if (turn % 5 === 0) {
      return { type: 'Special' as const, value: 25, description: 'ULTIMATE EXECUTION' };
    }
    const roll = Math.random();
    if (roll < 0.65) return { type: 'Attack' as const, value: 8, description: 'Standard Strike' };
    if (roll < 0.8) return { type: 'Special' as const, value: 5, description: 'Restoration' }; 
    return { type: 'Defend' as const, value: 3, description: 'Energy Shield' }; 
  }, []);

  const addLog = useCallback((msg: string) => {
    setLogs(prev => [msg, ...prev].slice(0, 5));
  }, []);

  const addNotification = useCallback((text: string) => {
    const id = Math.random().toString(36).substr(2, 9);
    setNotifications(prev => [{ id, text }, ...prev]);
    setTimeout(() => {
      setNotifications(prev => prev.filter(n => n.id !== id));
    }, 3000);
  }, []);

  const triggerDamagePopup = useCallback((value: number, target: 'Player' | 'Enemy', isHeal = false) => {
    const id = Math.random().toString(36).substr(2, 9);
    const popup: DamagePopup = {
      id,
      value,
      isHeal,
      x: target === 'Enemy' ? 50 : 20, 
      y: target === 'Enemy' ? 40 : 10
    };
    setDamagePopups(prev => [...prev, popup]);
    setTimeout(() => {
      setDamagePopups(prev => prev.filter(p => p.id !== id));
    }, 1000);
  }, []);

  // 战斗初始化
  useEffect(() => {
    const isBoss = run.currentNode === 10;
    const enemyKey = isBoss ? 'boss' : (run.currentNode % 2 === 0 ? 'specter' : 'executioner');
    const enemyData = ENEMIES[enemyKey];
    
    let mechanism: AnomalyMechanism = 'None';
    let monsterHpBonus = 1;
    if (run.currentRoomType === 'Event') {
      const mechanisms: AnomalyMechanism[] = ['TwinPairCombo', 'CriticalStrike', 'WeakHand', 'Vampiric'];
      mechanism = mechanisms[Math.floor(Math.random() * mechanisms.length)];
      if (mechanism === 'TwinPairCombo') monsterHpBonus = 1.3;
    }

    const initialDeck = shuffle([...run.deck]);
    const initialDrawSize = run.relics.includes('old_card_box') ? 6 : 5;
    const cardsToDraw = mechanism === 'WeakHand' ? initialDrawSize - 1 : initialDrawSize;

    const hand = drawXCards(cardsToDraw);

    let startHp = run.currentHp;
    if (run.relics.includes('gambler_coin')) startHp -= 2;

    setBattle({
      enemy: { 
        ...enemyData, 
        hp: Math.floor(enemyData.maxHp * monsterHpBonus), 
        maxHp: Math.floor(enemyData.maxHp * monsterHpBonus),
        intent: generateEnemyIntent(1)
      },
      playerHp: Math.max(startHp, 1),
      hand,
      drawPile: initialDeck.slice(cardsToDraw),
      discardPile: [],
      currentRedraws: 2,
      bonusRedrawsNextTurn: 0,
      armor: 0,
      turn: 1,
      comboPreview: null,
      selectedCardIds: [],
      mechanism,
      consecutivePairs: 0,
      firstStraightUsed: false
    });
    
    addLog(`Battle starts! Node ${run.currentNode}`);
    if (mechanism !== 'None') addNotification(`MODIFIER: ${mechanism.toUpperCase()}`);
  }, [run.currentNode, run.currentRoomType, run.deck, run.relics, run.currentHp, generateEnemyIntent, addLog, addNotification]);

  const sortedHand = useMemo(() => {
    if (!battle) return [];
    return [...battle.hand].sort((a, b) => b.value - a.value);
  }, [battle?.hand]);

  const toggleSelect = useCallback((id: string, forceMode?: 'select' | 'deselect') => {
    setBattle(prev => {
      if (!prev) return prev;
      const isCurrentlySelected = prev.selectedCardIds.includes(id);
      
      let nextSelection = prev.selectedCardIds;
      if (forceMode === 'select' && !isCurrentlySelected) {
        nextSelection = [...prev.selectedCardIds, id];
      } else if (forceMode === 'deselect' && isCurrentlySelected) {
        nextSelection = prev.selectedCardIds.filter(sid => sid !== id);
      } else if (!forceMode) {
        nextSelection = isCurrentlySelected 
          ? prev.selectedCardIds.filter(sid => sid !== id)
          : [...prev.selectedCardIds, id];
      }

      if (nextSelection === prev.selectedCardIds) return prev;
      
      const selectedCards = prev.hand.filter(c => nextSelection.includes(c.id));
      const comboPreview = detectCombo(selectedCards);

      // 核对无效点数 (防偷懒业务引擎)
      let invalid = false;
      if (selectedCards.length > 0 && comboPreview.type !== 'Straight' && comboPreview.type !== 'StraightFlush') {
        const counts: Record<number, number> = {};
        selectedCards.forEach(c => counts[c.value] = (counts[c.value] || 0) + 1);
        if (Object.values(counts).some(v => v === 1) && Object.keys(counts).length > 1) invalid = true;
      }

      setIsInvalidDeck(invalid);
      setIsInvalidCombo(false); // 改动三：选中状态变化立刻重置无效警告状态
      setShowInvalidFeedback(false); 

      return { ...prev, selectedCardIds: nextSelection, comboPreview };
    });
  }, []);

  const batchTogglePin = useCallback(() => {
    if (!battle || battle.selectedCardIds.length === 0) return;

    setBattle(prev => {
      if (!prev) return prev;
      const anyUnpinned = prev.hand.some(c => prev.selectedCardIds.includes(c.id) && !c.isPinned);
      
      let nextHand = [...prev.hand];
      if (anyUnpinned) {
        let normalPinned = prev.hand.filter(c => c.isPinned && c.suite !== 'Special').length;
        let specialPinned = prev.hand.filter(c => c.isPinned && c.suite === 'Special').length;

        nextHand = prev.hand.map(c => {
          if (!prev.selectedCardIds.includes(c.id) || c.isPinned) return c;
          const isSpecial = c.suite === 'Special';
          // 2x2 锁定限制
          if (isSpecial ? specialPinned < 2 : normalPinned < 2) {
             if (isSpecial) specialPinned++; else normalPinned++;
             return { ...c, isPinned: true };
          }
          return c;
        });
        addNotification("BATCH PINNED");
      } else {
        nextHand = prev.hand.map(c => prev.selectedCardIds.includes(c.id) ? { ...c, isPinned: false } : c);
        addNotification("BATCH UNPINNED");
      }

      return { ...prev, hand: nextHand, selectedCardIds: [] };
    });
  }, [battle, addNotification]);

  const handleRedraw = useCallback(() => {
    if (!battle || battle.currentRedraws <= 0 || battle.selectedCardIds.length === 0) return;

    const indices = battle.hand.reduce((acc: number[], c, i) => {
      if (battle.selectedCardIds.includes(c.id) && !c.isPinned) acc.push(i);
      return acc;
    }, []);

    if (indices.length === 0) return;

    const discarded = indices.map(i => battle.hand[i]);
    let drawPile = [...battle.drawPile];
    let discardPile = [...battle.discardPile, ...discarded];

    if (drawPile.length < indices.length) {
      drawPile = shuffle([...drawPile, ...discardPile]);
      discardPile = [];
    }

    const nextHand = [...battle.hand];
    indices.forEach((idx, i) => {
      nextHand[idx] = drawPile[i];
    });

    setBattle(prev => prev ? ({
      ...prev,
      hand: nextHand,
      drawPile: drawPile.slice(indices.length),
      discardPile,
      currentRedraws: prev.currentRedraws - 1,
      selectedCardIds: [],
      comboPreview: null
    }) : null);

    addLog(`Redraw used. ${indices.length} cards exchanged.`);
  }, [battle, addLog]);

  const executeEnemyTurn = useCallback((curr: BattleState, comboType: string, pairs: number, draw: number) => {
    const { type, value, description } = curr.enemy.intent;
    let hpDmg = 0;
    let armor = curr.armor;

    if (type === 'Attack' || (type === 'Special' && value === 25)) {
      let dmg = value;
      if (curr.mechanism === 'CriticalStrike') dmg = Math.floor(dmg * 1.5);
      const absorb = Math.min(armor, dmg);
      armor -= absorb;
      hpDmg = dmg - absorb;
      triggerDamagePopup(dmg, 'Player');
      setIsPlayerShaking(true);
      setTimeout(() => setIsPlayerShaking(false), 500);
    } else if (type === 'Special' && value === 5) {
      triggerDamagePopup(5, 'Enemy', true);
    }

    const nextHp = Math.max(0, curr.playerHp - hpDmg);
    if (nextHp <= 0) { onDefeat(); return; }

    const newDraw = drawXCards(5);
    const pinned = curr.hand.filter(c => c.isPinned);
    // 脸黑重抽算法保持一致
    const nextHand = [...pinned.map(c => ({ ...c, isPinned: c.suite === 'Special' })), ...newDraw];

    const t = curr.turn + 1;
    setBattle(prev => prev ? ({
      ...prev,
      playerHp: nextHp,
      enemy: { ...prev.enemy, hp: Math.min(prev.enemy.maxHp, prev.enemy.hp + (type === 'Special' ? 5 : 0)), 
               armor: prev.enemy.armor + (type === 'Defend' ? 15 : 0), 
               intent: generateEnemyIntent(t) },
      armor,
      hand: nextHand,
      turn: t,
      currentRedraws: 2 + draw,
      selectedCardIds: [],
      comboPreview: null,
      consecutivePairs: pairs,
      firstStraightUsed: prev.firstStraightUsed || comboType === 'Straight'
    }) : null);
  }, [onDefeat, generateEnemyIntent, triggerDamagePopup]);

  const handlePlay = useCallback(() => {
    if (!battle || battle.selectedCardIds.length === 0) return;

    // 改动二：出牌错误拦截提示逻辑
    if (!battle.comboPreview || isInvalidDeck) {
      addNotification("无效的牌型!");
      setIsInvalidCombo(true);
      setInvalidTriggerCount(prev => prev + 1);
      return;
    }

    const { multiplier, type, displayName } = battle.comboPreview;
    const selected = battle.hand.filter(c => battle.selectedCardIds.includes(c.id));
    
    let heal = 0, shield = 0, draw = 0;
    const effects = selected.filter(c => c.suite === 'Special').reduce((acc: Record<string, number>, c) => {
        acc[c.effect!] = (acc[c.effect!] || 0) + 1; return acc;
    }, {});

    if (effects['Heal']) heal = effects['Heal'] === 1 ? 5 : 15;
    if (effects['ArmorBuff']) shield = effects['ArmorBuff'] === 1 ? 4 : 12;
    if (effects['GainExtraTidy']) draw = effects['GainExtraTidy'] === 1 ? 1 : 2;

    let base = selected.filter(c => c.suite !== 'Special').reduce((s, c) => s + c.value, 0);
    let mult = multiplier;
    if (run.relics.includes('flow_emblem') && type === 'Straight') mult *= 1.15;
    if (run.relics.includes('twin_ring') && type === 'Pair') mult *= 1.1;
    if (run.relics.includes('black_powder') && type === 'Bomb') mult *= 1.2;
    if (run.relics.includes('desolation_seal') && type === 'Bomb' && battle.playerHp < run.maxHp * 0.3) mult *= 1.2;

    let totalDmg = Math.floor(base * mult);
    if (battle.mechanism === 'CriticalStrike') totalDmg = Math.floor(totalDmg * 1.5);
    if (battle.mechanism === 'WeakHand') totalDmg += selected.length * 2;
    if (battle.mechanism === 'TwinPairCombo' && type === 'TwoPair') totalDmg *= 2;

    let pairs = battle.consecutivePairs;
    if (type === 'Pair') { if (++pairs >= 2 && run.relics.includes('steady_chip')) heal += 3; } else pairs = 0;

    if (type === 'Straight' && run.relics.includes('gale_gloves') && !battle.firstStraightUsed) draw += 1;
    if (type === 'Bomb') { 
        setShowExplosion(true); setTimeout(() => setShowExplosion(false), 1500);
        if (run.relics.includes('fuse_amulet')) draw += 1;
    }
    if (battle.mechanism === 'Vampiric') heal += Math.floor(totalDmg * 0.1);

    const nextP = Math.min(battle.playerHp + heal, run.maxHp);
    let eArmor = battle.enemy.armor;
    let hpDmg = totalDmg;
    if (eArmor > 0) { const abs = Math.min(eArmor, totalDmg); eArmor -= abs; hpDmg -= abs; triggerDamagePopup(abs, 'Enemy'); }

    // 随机未命中惩罚逻辑
    if (Math.random() < 0.03) {
        addNotification("MISS!"); triggerDamagePopup(0, 'Enemy');
        const update = { ...battle, playerHp: nextP, armor: battle.armor + shield, enemy: { ...battle.enemy, armor: eArmor } };
        setBattle(update); setTimeout(() => executeEnemyTurn(update, type, pairs, draw), 1000);
        return;
    }

    const nextE = Math.max(0, battle.enemy.hp - hpDmg);
    if (hpDmg > 0) triggerDamagePopup(hpDmg, 'Enemy');
    if (heal > 0) triggerDamagePopup(heal, 'Player', true);
    setBattlePrompt(`${displayName}${heal > 0 ? ' +Mend' : ''}${shield > 0 ? ' +Guard' : ''}`);
    setTimeout(() => setBattlePrompt(null), 1500);

    if (nextE <= 0) {
        setBattle(prev => prev ? ({ ...prev, enemy: { ...prev.enemy, hp: 0 }, playerHp: nextP }) : null);
        setTimeout(() => setVictoryLoot({ gold: 15, playerHp: nextP }), 800);
        return;
    }

    const update = { ...battle, enemy: { ...battle.enemy, hp: nextE, armor: eArmor }, playerHp: nextP, armor: battle.armor + shield };
    setBattle(update);
    setTimeout(() => executeEnemyTurn(update, type, pairs, draw), 1000);
  }, [battle, isInvalidDeck, run, addNotification, triggerDamagePopup, executeEnemyTurn]);

  return {
    battle,
    isEnemyShaking,
    isPlayerShaking,
    showExplosion,
    logs,
    damagePopups,
    notifications,
    isHandShaking,
    isInvalidDeck,
    isInvalidCombo,
    invalidTriggerCount,
    showInvalidFeedback,
    victoryLoot,
    battlePrompt,
    isDragging,
    setIsDragging,
    dragMode,
    setDragMode,
    hoveredCardId,
    setHoveredCardId,
    sortedHand,
    toggleSelect,
    batchTogglePin,
    handleRedraw,
    handlePlay,
    setVictoryLoot
  };
}
